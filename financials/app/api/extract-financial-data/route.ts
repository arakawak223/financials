...
3. 勘定科目明細（account_details）を抽出：
   - **【新規追加】「売上高の内訳」**がある場合、各項目を抽出してください（例：製品売上、商品売上、サービス売上など）
   - **【新規追加】「売上原価の内訳」**がある場合、各項目を抽出してください（例：製品原価、商品原価、材料費、外注費など）
   - 「販売費及び一般管理費の明細」がある場合、各項目（減価償却費、給料手当、地代家賃など）を抽出
   - 「製造原価報告書」の明細がある場合、各項目を抽出
   - 各明細は以下の形式で返してください：
     * account_category: 区分（"売上高" または "売上原価" または "selling_general_admin" または "cost_of_sales"）
